...
static PyObject *dxc_compile(PyObject *self, PyObject *args) {
  ...
  PyObject *py_target;
  if (!PyArg_ParseTuple(args, "s*UiU", &view, &py_entry_point,
                        &shader_binary_type, &py_target))
    return NULL;

  static DxcCreateInstanceProc dxcompiler_lib_create_instance_proc = nullptr;
  if (!dxcompiler_lib_create_instance_proc) {
    dxcompiler_lib_create_instance_proc =
        (DxcCreateInstanceProc)dlsym(RTLD_DEFAULT, "DxcCreateInstance");
    if (!dxcompiler_lib_create_instance_proc)
  ...
  HRESULT hr = dxcompiler_lib_create_instance_proc(
      CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void **)&dxc_library);
  if (hr != S_OK)
    return dxc_generate_exception(hr, "unable to create DXC library instance");

  // Create DXC compiler
  IDxcCompiler *dxc_compiler = nullptr;
  hr = dxcompiler_lib_create_instance_proc(
      CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void **)&dxc_compiler);
  if (hr != S_OK) {
    dxc_library->Release();
  ...
  hr = dxc_library->CreateBlobWithEncodingOnHeapCopy(view.buf, (UINT32)view.len,
                                                     CP_UTF8, &blob_source);
  if (hr != S_OK) {
  ...
  std::vector<const wchar_t *> arguments = {L"-spirv",
  ...
  PyObject *py_compiled_blob =
      PyBytes_FromStringAndSize((const char *)compiled_blob->GetBufferPointer(),
...
static PyMethodDef compushady_backends_dxc_methods[] = {
    {"compile", (PyCFunction)dxc_compile, METH_VARARGS,
     "Compile an HLSL shader to SPIR‑V"},
