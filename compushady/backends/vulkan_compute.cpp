...
PyObject *vulkan_Compute_dispatch(vulkan_Compute *self, PyObject *args) {
    ...
    if (push.len > 0) {
        if (push.len > self->push_constant_size || (push.len % 4) != 0) {
            ...
            return PyErr_Format(PyExc_ValueError,
                "Invalid push constant size: %u, expected max %u with 4 bytes alignment",
    ...
    if (res != VK_SUCCESS)
        return PyErr_Format(PyExc_RuntimeError, "Dispatch submission failed: %d", res);
...
/* ----------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
PyObject *vulkan_Compute_dispatch_indirect(vulkan_Compute *self, PyObject *args) {
