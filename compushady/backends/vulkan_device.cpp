...
vulkan_Device *vulkan_Device_get_device(vulkan_Device *self) {
    ...
    VkQueueFamilyProperties *qfs =
        (VkQueueFamilyProperties *)PyMem_Malloc(qf_count * sizeof(VkQueueFamilyProperties));
...
PyObject *vulkan_Device_create_heap(vulkan_Device *self, PyObject *args) {
    ...
    if (!py_heap) return PyErr_NoMemory();
    memset((char*)py_heap + sizeof(PyObject), 0, sizeof(vulkan_Heap) - sizeof(PyObject));
    py_heap->py_device = py_device;
    Py_INCREF(py_heap->py_device);

    VkMemoryAllocateInfo alloc_info = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
    alloc_info.allocationSize = size;
    alloc_info.memoryTypeIndex = get_memory_type_index(&self->mem_props, mem_flag);

    VkResult result = vkAllocateMemory(py_device->device, &alloc_info, NULL, &py_heap->memory);
...
PyObject *vulkan_Device_create_buffer(vulkan_Device *self, PyObject *args) {
    ...
    switch (heap_type) {
        ...
        case 1: mem_flag = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT; break; // UPLOAD
        case 2: mem_flag = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT; break; // READBACK
        default: return PyErr_Format(Compushady_BufferError, "Invalid heap type: %d", heap_type);
    }

    vulkan_Resource *py_resource = PyObject_New(vulkan_Resource, &vulkan_Resource_Type);
    if (!py_resource) return PyErr_NoMemory();
    memset((char*)py_resource + sizeof(PyObject), 0, sizeof(vulkan_Resource) - sizeof(PyObject));
    py_resource->py_device = py_device;
    Py_INCREF(py_resource->py_device);

    VkResult result = vkCreateBuffer(py_device->device, &buffer_info, NULL, &py_resource->buffer);
    if (result != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(py_device->device, py_resource->buffer, &mem_req);

    if (sparse) {
        py_resource->tile_width = (uint32_t)mem_req.alignment;
        py_resource->tile_height = 1;
        py_resource->tile_depth = 1;
        py_resource->tiles_x = (uint32_t)ceil((double)mem_req.size / mem_req.alignment);
        ...
        alloc_info.memoryTypeIndex = get_memory_type_index(&self->mem_props, mem_flag);
        result = vkAllocateMemory(py_device->device, &alloc_info, NULL, &py_resource->memory);
        if (result != VK_SUCCESS) {
            Py_DECREF(py_resource);
    ...
    if (format > 0) {
        ...
        result = vkCreateBufferView(py_device->device, &view_info, NULL, &py_resource->buffer_view);
        if (result != VK_SUCCESS) {
            Py_DECREF(py_resource);
...
PyObject *vulkan_Device_create_texture2d(vulkan_Device *self, PyObject *args) {
    ...
    if (sparse) {
        ...
        for (auto &req : sparse_reqs) {
            if (req.formatProperties.aspectMask & VK_IMAGE_ASPECT_COLOR_BIT) {
                ...
                py_resource->tiles_x = (uint32_t)ceil((double)width / py_resource->tile_width);
                py_resource->tiles_y = (uint32_t)ceil((double)height / py_resource->tile_height);
                py_resource->tiles_z = 1;
                break;
            }
        }
        ...
        VkMemoryAllocateInfo alloc_info = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
        alloc_info.allocationSize = mem_req.size;
        alloc_info.memoryTypeIndex = get_memory_type_index(&self->mem_props,
                                                           VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        VkResult result = vkAllocateMemory(py_device->device, &alloc_info, NULL, &py_resource->memory);
        if (result != VK_SUCCESS) {
            Py_DECREF(py_resource);
    ...
    view_info.viewType = slices > 1 ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D;
    view_info.format = vulkan_formats[format].first;
    view_info.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    view_info.subresourceRange.levelCount = 1;
    view_info.subresourceRange.layerCount = slices;

    VkResult result = vkCreateImageView(py_device->device, &view_info, NULL, &py_resource->image_view);
    if (result != VK_SUCCESS) {
        Py_DECREF(py_resource);
    ...
    if (!vulkan_texture_set_layout(py_device, py_resource->image,
                                   VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, slices)) {
        Py_DECREF(py_resource);
...
#define COMPUSHADY_VULKAN_SAMPLER_ADDRESS_MODE(ret, var, field) \
    if (var == 0) ret = VK_SAMPLER_ADDRESS_MODE_REPEAT;         \
    else if (var == 1) ret = VK_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT; \
    else if (var == 2) ret = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE; \
    else return PyErr_Format(Compushady_SamplerError, "unsupported address mode for " field)
...
PyObject *vulkan_Device_create_sampler(vulkan_Device *self, PyObject *args) {
    ...
    COMPUSHADY_VULKAN_SAMPLER_ADDRESS_MODE(sampler_info.addressModeU, addr_u, "U");
    COMPUSHADY_VULKAN_SAMPLER_ADDRESS_MODE(sampler_info.addressModeV, addr_v, "V");
    COMPUSHADY_VULKAN_SAMPLER_ADDRESS_MODE(sampler_info.addressModeW, addr_w, "W");
    ...
    vulkan_Sampler *py_sampler = PyObject_New(vulkan_Sampler, &vulkan_Sampler_Type);
    if (!py_sampler) return PyErr_NoMemory();
    memset((char*)py_sampler + sizeof(PyObject), 0, sizeof(vulkan_Sampler) - sizeof(PyObject));
    py_sampler->py_device = py_device;
    Py_INCREF(py_sampler->py_device);

    VkResult result = vkCreateSampler(py_device->device, &sampler_info, NULL, &py_sampler->sampler);
    if (result != VK_SUCCESS) {
        Py_DECREF(py_sampler);
        return PyErr_Format(Compushady_SamplerError, "unable to create vulkan Sampler");
    }
...
PyObject *vulkan_Device_create_compute(vulkan_Device *self, PyObject *args, PyObject *kwds) {
    ...
    Py_buffer view;
    PyObject *cbv_list = NULL, *srv_list = NULL, *uav_list = NULL, *samplers_list = NULL;
    ...
    if (!compushady_check_descriptors(&vulkan_Resource_Type, cbv_list, cbv,
    ...
    if (bindless == 0) {
        ...
        binding_offset = 1024;
        for (auto *res : srv) {
            VkDescriptorType type = res->buffer ? (res->buffer_view ? VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER : VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) : VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
        ...
        binding_offset = 2048;
        for (auto *res : uav) {
            VkDescriptorType type = res->buffer ? (res->buffer_view ? VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) : VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
            descriptors[type].push_back(res);
            VkDescriptorSetLayoutBinding lb = {};
            lb.binding = binding_offset++;
            lb.descriptorCount = 1;
        ...
        if (cbv.size() > bindless || srv.size() > bindless || uav.size() > bindless) {
            PyBuffer_Release(&view);
            return PyErr_Format(PyExc_ValueError, "Too many initial resources for bindless");
        }
#ifdef VK_EXT_descriptor_indexing
    ...
    if (!py_device->features.shaderStorageImageReadWithoutFormat) {
        ...
        for (auto *res : uav) {
            if (res->image && (res->format == VK_FORMAT_B8G8R8A8_UNORM || res->format == VK_FORMAT_B8G8R8A8_SRGB)) {
                uint32_t *patched = vulkan_patch_spirv_unknown_uav((uint32_t *)view.buf, view.len, binding_offset);
                if (patched) {
    ...
    // Entry point
    const char *entry = vulkan_get_spirv_entry_point(shader_info.pCode, shader_info.codeSize);
    if (!entry) {
        if (shader_info.pCode != view.buf) PyMem_Free((void *)shader_info.pCode);
        PyBuffer_Release(&view);
    ...
    VkResult res = vkCreateShaderModule(py_device->device, &shader_info, NULL, &shader_module);
    if (shader_info.pCode != view.buf) PyMem_Free((void *)shader_info.pCode);
    PyBuffer_Release(&view);
    if (res != VK_SUCCESS) return PyErr_Format(Compushady_ComputeError, "Shader module creation failed");

    // Allocate Compute object
    vulkan_Compute *comp = PyObject_New(vulkan_Compute, &vulkan_Compute_Type);
    if (!comp) { vkDestroyShaderModule(py_device->device, shader_module, NULL); return PyErr_NoMemory(); }
    memset((char*)comp + sizeof(PyObject), 0, sizeof(vulkan_Compute) - sizeof(PyObject));
...
#endif
    if (vkCreateDescriptorSetLayout(py_device->device, &dsl_info, NULL, &comp->descriptor_set_layout) != VK_SUCCESS) {
        vkDestroyShaderModule(py_device->device, shader_module, NULL);
        Py_DECREF(comp);
        return PyErr_Format(Compushady_ComputeError, "Failed to create descriptor set layout");
    }
    ...
    if (vkCreatePipelineLayout(py_device->device, &pl_info, NULL, &comp->pipeline_layout) != VK_SUCCESS) {
        vkDestroyDescriptorSetLayout(py_device->device, comp->descriptor_set_layout, NULL);
        vkDestroyShaderModule(py_device->device, shader_module, NULL);
        Py_DECREF(comp);
        return PyErr_Format(Compushady_ComputeError, "Failed to create pipeline layout");
    }
    ...
    if (vkCreateComputePipelines(py_device->device, VK_NULL_HANDLE, 1, &cp_info, NULL, &comp->pipeline) != VK_SUCCESS) {
        vkDestroyPipelineLayout(py_device->device, comp->pipeline_layout, NULL);
        vkDestroyDescriptorSetLayout(py_device->device, comp->descriptor_set_layout, NULL);
        vkDestroyShaderModule(py_device->device, shader_module, NULL);
        Py_DECREF(comp);
        return PyErr_Format(Compushady_ComputeError, "Failed to create compute pipeline");
    }
...
#ifdef VK_EXT_descriptor_indexing
    if (bindless) dp_info.flags = VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT_EXT;
#endif
    if (vkCreateDescriptorPool(py_device->device, &dp_info, NULL, &comp->descriptor_pool) != VK_SUCCESS) {
        vkDestroyPipeline(py_device->device, comp->pipeline, NULL);
        vkDestroyPipelineLayout(py_device->device, comp->pipeline_layout, NULL);
        vkDestroyDescriptorSetLayout(py_device->device, comp->descriptor_set_layout, NULL);
        vkDestroyShaderModule(py_device->device, shader_module, NULL);
        Py_DECREF(comp);
        return PyErr_Format(Compushady_ComputeError, "Failed to create descriptor pool");
    }
    ...
    if (vkAllocateDescriptorSets(py_device->device, &ds_alloc, &comp->descriptor_set) != VK_SUCCESS) {
        ...
        vkDestroyShaderModule(py_device->device, shader_module, NULL);
        Py_DECREF(comp);
        return PyErr_Format(Compushady_ComputeError, "Failed to allocate descriptor set");
    }
    ...
    for (auto *res : srv) {
        ...
        w.dstBinding = binding_offset++;
        w.descriptorCount = 1;
        w.descriptorType = res->buffer ? (res->buffer_view ? VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER : VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) : VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
        if (res->buffer) {
            if (res->buffer_view) w.pTexelBufferView = &res->buffer_view;
            else w.pBufferInfo = &res->descriptor_buffer_info;
        } else {
    ...
    for (auto *res : uav) {
        ...
        w.dstBinding = binding_offset++;
        w.descriptorCount = 1;
        w.descriptorType = res->buffer ? (res->buffer_view ? VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) : VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        if (res->buffer) {
            if (res->buffer_view) w.pTexelBufferView = &res->buffer_view;
            else w.pBufferInfo = &res->descriptor_buffer_info;
        } else {
...
PyObject *vulkan_Device_create_swapchain(vulkan_Device *self, PyObject *args) {
    ...
    memset((char*)sc + sizeof(PyObject), 0, sizeof(vulkan_Swapchain) - sizeof(PyObject));
    ...
    if (vkCreateXlibSurfaceKHR(vulkan_instance, &surf_info, NULL, &sc->surface) != VK_SUCCESS) {
        Py_DECREF(sc);
        return PyErr_Format(Compushady_SwapchainError, "Failed to create Xlib surface");
    }

    VkBool32 supported;
    vkGetPhysicalDeviceSurfaceSupportKHR(self->physical_device, self->queue_family_index,
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(self->physical_device, sc->surface, &caps);
    ...
    if (result != VK_SUCCESS) {
        vkDestroySurfaceKHR(vulkan_instance, sc->surface, NULL);
        Py_DECREF(sc);
...
static PyMethodDef vulkan_Device_methods[] = {
    ...
    {"create_buffer", (PyCFunction)vulkan_Device_create_buffer, METH_VARARGS, NULL},
    {"create_texture2d", (PyCFunction)vulkan_Device_create_texture2d, METH_VARARGS, NULL},
    {"create_sampler", (PyCFunction)vulkan_Device_create_sampler, METH_VARARGS, NULL},
...
static PyMemberDef vulkan_Device_members[] = {
    ...
    {"dedicated_video_memory", T_ULONGLONG, offsetof(vulkan_Device, dedicated_video_memory), 0, NULL},
    {"dedicated_system_memory", T_ULONGLONG, offsetof(vulkan_Device, dedicated_system_memory), 0, NULL},
    {"shared_system_memory", T_ULONGLONG, offsetof(vulkan_Device, shared_system_memory), 0, NULL},
