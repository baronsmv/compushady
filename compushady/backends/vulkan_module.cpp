...
PyObject *vulkan_get_discovered_devices(PyObject *self, PyObject *args) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        memset((char*)dev + sizeof(PyObject), 0, sizeof(vulkan_Device) - sizeof(PyObject));
...
static PyMethodDef vulkan_module_methods[] = {
    ...
    {"get_shader_binary_type", (PyCFunction)vulkan_get_shader_binary_type, METH_NOARGS,
     "Returns the required shader binary type (SPIR-V = 1)"},
    {NULL, NULL, 0, NULL}
};
...
PyMODINIT_FUNC PyInit_vulkan(void) {
    ...
    Compushady_Texture2DError = PyErr_NewException("vulkan.Texture2DError", NULL, NULL);
    ...
    Compushady_ComputeError = PyErr_NewException("vulkan.ComputeError", NULL, NULL);
    PyModule_AddObject(m, "ComputeError", Compushady_ComputeError);
    Compushady_SwapchainError = PyErr_NewException("vulkan.SwapchainError", NULL, NULL);
    ...
    Compushady_SamplerError = PyErr_NewException("vulkan.SamplerError", NULL, NULL);
    PyModule_AddObject(m, "SamplerError", Compushady_SamplerError);
    Compushady_Texture1DError = PyErr_NewException("vulkan.Texture1DError", NULL, NULL);
    PyModule_AddObject(m, "Texture1DError", Compushady_Texture1DError);
    Compushady_Texture3DError = PyErr_NewException("vulkan.Texture3DError", NULL, NULL);
    ...
    // Register types
    if (PyType_Ready(&vulkan_Device_Type) < 0) return NULL;
    PyModule_AddObject(m, "Device", (PyObject *)&vulkan_Device_Type);

    if (PyType_Ready(&vulkan_Heap_Type) < 0) return NULL;
    PyModule_AddObject(m, "Heap", (PyObject *)&vulkan_Heap_Type);

    if (PyType_Ready(&vulkan_Resource_Type) < 0) return NULL;
    PyModule_AddObject(m, "Resource", (PyObject *)&vulkan_Resource_Type);

    if (PyType_Ready(&vulkan_Sampler_Type) < 0) return NULL;
    PyModule_AddObject(m, "Sampler", (PyObject *)&vulkan_Sampler_Type);

    if (PyType_Ready(&vulkan_Compute_Type) < 0) return NULL;
    PyModule_AddObject(m, "Compute", (PyObject *)&vulkan_Compute_Type);

    if (PyType_Ready(&vulkan_Swapchain_Type) < 0) return NULL;
