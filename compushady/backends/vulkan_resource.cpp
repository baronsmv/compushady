...
static void vulkan_Resource_dealloc(vulkan_Resource *self) {
    if (self->py_device) {
        ...
        if (self->image_view) vkDestroyImageView(device, self->image_view, NULL);
        if (self->buffer_view) vkDestroyBufferView(device, self->buffer_view, NULL);
        if (!self->py_heap && self->memory) vkFreeMemory(device, self->memory, NULL);
        if (self->image) vkDestroyImage(device, self->image, NULL);
        if (self->buffer) vkDestroyBuffer(device, self->buffer, NULL);
...
PyObject *vulkan_Resource_upload(vulkan_Resource *self, PyObject *args) {
    ...
    if (offset + view.len > self->size) {
        ...
        return PyErr_Format(PyExc_ValueError,
            "supplied buffer is bigger than resource size: (offset %llu) %llu (expected no more than %llu)",
...
PyObject *vulkan_Resource_copy_to(vulkan_Resource *self, PyObject *args) {
    ...
    if (dst->py_device != dev)
        return PyErr_Format(PyExc_ValueError, "Resources belong to different devices");
...
/* ----------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
PyObject *vulkan_Resource_download_texture(vulkan_Resource *self, PyObject *ignored) {
    if (!self->image) {
    ...
    if (vkAllocateMemory(dev->device, &alloc, NULL, &device_memory) != VK_SUCCESS) {
...
PyMethodDef vulkan_Resource_methods[] = {
    ...
    {"download", (PyCFunction)vulkan_Resource_download_texture, METH_NOARGS, NULL},
    {"upload_subresource", (PyCFunction)vulkan_Resource_upload_subresource, METH_VARARGS, NULL},
    {NULL}
};
