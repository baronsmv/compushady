...
static void vulkan_Swapchain_dealloc(vulkan_Swapchain *self) {
    if (self->py_device) {
        ...
        if (self->copy_semaphore) vkDestroySemaphore(dev, self->copy_semaphore, NULL);
...
PyObject *vulkan_Swapchain_present(vulkan_Swapchain *self, PyObject *args) {
    PyObject *tex_obj;
    uint32_t x = 0, y = 0;
    if (!PyArg_ParseTuple(args, "O|II", &tex_obj, &x, &y))
        return NULL;
    ...
    if (res != VK_SUCCESS && res != VK_SUBOPTIMAL_KHR) {
        return PyErr_Format(PyExc_RuntimeError, "Failed to acquire swapchain image: %d", res);
    }
    ...
    region.extent.width = Py_MIN(tex->image_extent.width, self->image_extent.width - x);
    region.extent.height = Py_MIN(tex->image_extent.height, self->image_extent.height - y);
...
PyObject *vulkan_Swapchain_needs_recreation(vulkan_Swapchain *self, PyObject *ignored) {
    if (self->suboptimal || self->out_of_date) Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
