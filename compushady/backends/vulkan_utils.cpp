...
const char *vulkan_get_spirv_entry_point(const uint32_t *words, uint64_t len) {
    ...
    if (words[0] != 0x07230203) return NULL; // SPIR-V magic
    ...
    while (offset < words_num) {
        ...
        if (opcode == 0x0F && (offset + size < words_num) && words[offset + 1] == 5) {
            if (size > 3) {
...
/* ----------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
uint32_t vulkan_get_memory_type_index_by_flag(VkPhysicalDeviceMemoryProperties *mem_props,
...
PyObject *vulkan_instance_check(void) {
    ...
    for (auto &e : exts) {
        ...
        if (vulkan_debug && !strcmp(e.extensionName, VK_EXT_DEBUG_UTILS_EXTENSION_NAME)) {
    ...
    if (res != VK_SUCCESS) {
        PyErr_Format(PyExc_RuntimeError, "Failed to create Vulkan instance: %d", res);
        return NULL;
    }
...
bool compushady_check_descriptors(
    PyTypeObject *res_type, PyObject *py_cbv, std::vector<vulkan_Resource *> &cbv,
...
{
    auto check_list = [&](PyObject *list, std::vector<vulkan_Resource *> &vec, const char *name) -> bool {
    ...
    auto check_samplers = [&](PyObject *list, std::vector<vulkan_Sampler *> &vec) -> bool {
        if (!list || list == Py_None) return true;
        if (!PyList_Check(list)) {
    ...
    if (!check_list(py_cbv, cbv, "cbv")) return false;
    if (!check_list(py_srv, srv, "srv")) return false;
    if (!check_list(py_uav, uav, "uav")) return false;
    if (!check_samplers(py_samplers, samplers)) return false;
    return true;
}